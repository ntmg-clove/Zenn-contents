---
title: "AWKの魅力を伝える: 「同形式のcsvの分割・統合」"
emoji: "✨"
...
今回はawkを用いた「**csvファイルの分割・統合**」をテーマにしています。
以下のテクニックを扱います。
...
## 使用するテストデータ
以前の記事で使用した天気データを加工して、日付ごとの平均気温のデータを1月～3月分作成しました。
例によって長いので、折りたたんでいます。
...
# 実践例6: 同じ形式のcsvファイルを1つにまとめる
同じ形式のcsvファイルが複数あり、これらを1つのファイルにまとめるというものです。
複数ファイルを連結する場合は「catコマンド」を利用するのが一般的ですが、以下のようなヘッダ付きのcsvファイルを連結する場合に「**headerは最初のみ付けて後のファイルでは無視したい**」という状況もあると思います。
...
これまでの説明では`cat`等で出力をした結果を`|`で`awk`に繋いで処理をしていましたが、引数としてファイルを与えることも出来ます。^[cat/grep/sed/cut/sort等、多くのLinuxコマンドがどちらでも受け取れるようになっていると思います。]
...
```bash:6-別解1
ls date2temp-avg_??.csv | xargs -I@ sed 1d @ | sed 1i`head -1 date2temp-avg_01.csv`
ls date2temp-avg_??.csv | xargs -I@ sed 1d @ | cat <(head -1 date2temp-avg_01.csv) -
```
...
## 別の方法2: awkによる重複排除を利用する（同一データがない想定）
今回テストで扱っているcsvファイルは日付を1列目に持っており、これらは重複することがありません。つまり、データ行において$1が同じ値となることがあり得ません。
このようなケースでは、**awkによる重複排除**が生かせます。
```bash:6-別解3
cat date2temp-avg_??.csv | awk '!a[$1]++' > date2temp_summary.csv
```
ヘッダ行は全く同じ値となるので最初のみ出力されます。データ行は、すべて異なる値が$1に格納されている想定のため、すべて出力されることになります。
...
今回の実践例7では、月毎にファイルを振り分けるためにデータ行の値を元にしているので、splitコマンドを使って狙い通りに振り分けるのは難しいです。^[月ごとに何日まであるかが同じであればsplitで容易に出来そうですが…]
...
awkではこれらのような複雑な処理も最小限の仕組みを活用することでこなせる場合が多いです。
