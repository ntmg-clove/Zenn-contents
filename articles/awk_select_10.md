...
その結果、ある程度は触れるようになってきたので、awkが活用できる実践例をいくつか紹介できればとおもいます。
...
今回は、集計などの説明をする際に以下のファイルを使用します。長くなるのが嫌なので省略しています。このデータは次回以降でも使うことにします。

:::details クリックして表示 
```txt:openweatherdata.txt
...
```
といった使い方が出来ます。[^6]
[^6]: 少し面倒くさいですが、awkでも連想配列を用いて集計は出来ます。

状況によって、`awk`、`sort -u`、`sort | uniq`を上手く使い分けられると良いと思います。

## 発展: 特定フィールドのみの重複排除
awkでは複数フィールドの関連した重複排除なども可能です。
行全体ではなく、特定フィールドのみの重複を確認したい場合はawkを使用するとスマートです。

この例では`openweatherdata.csv`を使用します。3列目は天気の分類、4列目は具体的な天気の説明が記述されているので、4列目を元に「データ内に存在する天気の説明のパターン」を抽出してみましょう。
```bash
...
この処理そのものに実用性があるかはさておき、余計な情報の出力を削るといったことを実現するケースはそれなりにあると思います。ログのステータス変化のない部分を削るなど？
さすがにgrep/sedでこのような処理をするのは苦しいのではないでしょうか。
そもそもgrep/sedは改行を跨いでの処理が苦手という弱点があります。[^7]
...
出力結果は長いので折りたたんでいます。少し見やすくなりましたね。
:::details クリックして表示
...
awkで行っている処理が少し複雑だと思うので、注釈とインデントを付けてみました。[^8]
...
ちなみにこのデータはヘッダがあるので、本来はNR2からスタートさせるのが適切だと思います。
今回のケースはどちらでも問題ないので横着しています。
...
次回はawkならではの集計処理などの観点で執筆したいと思います。
少しでもawkの魅力が伝われば良いと思っています。