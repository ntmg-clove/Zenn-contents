---
title: "awkの魅力を伝える: 「重複排除」「前行と比較しながらの条件分岐」"
emoji: "🕌"
...
`oslist.txt`に登場するOSの種類を列挙したいとします。
```bash:実行コマンド
...
試しに、`oslist.txt`に対してuniqを直接適用してみます。
```bash:実行コマンド
cat oslist.txt | uniq
```
...
[^6]: 少し面倒ですが、awkでも連想配列を用いて集計することが出来ます。
