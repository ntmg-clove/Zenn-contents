...
18948
```
パターンを`NR>1`としているのでヘッダ行は含めず^[実はこの例に関しては`NR>1`は不要です。ヘッダが数字でなければsumに足しても0として扱われるみたいですが、ヘッダを含むcsvを扱う場合は癖として`NR>1`を書くようにした方が良いと思います。]、に2行目以降の`$7`の金額の値を変数`sum`に加算して、最後にENDブロックで`sum`の値を出力します。
...
この例では条件分岐の種類はバス、電車の2種類のみですが、分岐の項目が多くなってきた場合にコードが長くなってしまいます。その場合は、実践例5で紹介するような連想配列・for文を使用する方法が有効です。
...
```
C言語やJavaなど多くのプログラミング言語が、同様の書式で3項演算子の機能をサポートしています。^[便利な3項演算子ですが、ネストをしたり条件や値などが長くなったり複雑になるようなものは、可読性を落とすことからプログラム中で積極的に使用されることが少ないように思います。実践例4のようにシンプルな用途で使用するのが吉だと思います。]
...
例えば、実践例3-1の処理はawkを使わずに処理するならば、文字列を整形して計算式にしてそこからbcに渡すといった方法が考えられます。
以下はsedを用いた例です。^[私はsedに慣れているので大抵文字列の整形にはsedを使いがちなのですが、多分もっと良い方法はあると思います。]

```bash:実践例3-1 sedとbcを用いた例
cat transport-inputdata-22-1.csv | sed 1d | sed -E 's/.*,([^,]*)$/\1/' | sed -z 's/\n/+/g;s/+$/\n/' | bc
```

このワンライナーは、sedや正規表現に慣れていないと処理が難しいかもしれません。また、より複雑な書式のファイルやより複雑な操作になった際には対応が難しくなるでしょう。実践例5のようにグループごとに平均値を出力などのようになると、同じような方法は通用しないでしょう。
もちろんLinuxコマンドで行うことを諦め、Pythonなどの数値計算が得意なスクリプト言語に任せてしまうのも手だと思います。筆者もawkを使う前はこのような処理はすべてPythonを使って処理していました。awkがある程度使えるようになると、この程度の計算にわざわざスクリプトファイルを書く必要がなくなってので、そういう面でもawkの利用価値を感じています。
